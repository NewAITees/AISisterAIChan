...
public class VRMController : MonoBehaviour
{
    ...
    private void StartIdle(Vrm10Instance instance)
    {
        ...
        LogAnimation($"IdleStart instance={instance.name} clip={idleClip.name}");
    }
    ...
    private static void LogAnimation(string message)
    {
        try
        {
            var logPath = GetAnimationLogPath();
            var line = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}] {message}\n";
            File.AppendAllText(logPath, line);
        }
        catch (Exception ex)
        {
            Debug.LogWarning("Animation log failed: " + ex.Message);
        }
    }

    private static string GetAnimationLogPath()
    {
        var logDir = Path.GetFullPath(Path.Combine(Application.dataPath, "..", "Logs"));
        Directory.CreateDirectory(logDir);
        return Path.Combine(logDir, "vrm_animation.log");
    }
}
